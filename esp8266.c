...
u8 parse_error() {
    volatile char *pos = strstr(g_buffer, error_answer);
    if (pos == 0) {
        return 1; // No match
    }
    memset(g_buffer, 0 , received_string_length);
...
u8 parse_AT_CWMODE() {
    volatile char *pos = strstr(g_buffer, ok_answer);
    u8 len = ok_answer_length;
    if (pos == 0) {
        // No match
        char *no_change = "no change";
        pos = strstr(g_buffer, no_change);
        len = strlen(no_change);
    }
...
u8 parse_AT_CIPSTART() {
    ...
    if (state == 0) {
        ...
        u8 linked_length = strlen(linked);

        volatile char *p1 = strstr(g_buffer, linked);
        if (p1 != 0) {
            state = 2; // Matched
        }
...
u8 parse_AT_CIPMUX() {
    if (parse_ok(g_buffer) != 0) {
        // no "ok", check is link is established already

        char builded[] = "link is builded\r\n";
        u8 builded_length = strlen(builded);

        volatile char *p1 = strstr(g_buffer, builded);
